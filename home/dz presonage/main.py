...
genderList = ["Мужской", "Женский"] # массив для выбора пола
raceList = ["Человек", "Эльф", "Гном", "Орк"] #  массив для выбора расы
roleList = ["Воин", "лучник", "Маг"] #  массив для выбора роли
textRace = ""
textRole = "" # переменные для вывода высех возможных рас

for i in range (0 , len(raceList)):
    textRace += f"{i} - {raceList[i]}\n" # повторять от 0 до количества рас 
textRace += f"{len(raceList)} - назад \n"
#print("регистрация персонажа")
reg = False #  глобальная регистрация / все переменные которые имеют приставку reg отвечают   
while reg == False:
    reg_gender = False
